...
public class UsersServiceImpl implements UsersService{
	...
	public Map<String, Object> isExistId(String inputId) {
		...
		return map;
	}//Map에 {"isExist":true} or {"isExist":false} 이렇게 응답 됨.(따로 jsp 페이지를 만들지 않아도)
	@Override
	...
	public String saveProfileImage(HttpServletRequest request, MultipartFile mFile) {
		...
		//원본 파일명(파일을 업로드 했을 때 저장한 파일명대로 목록이 보여져야 할 때)
		...
		//파일 시스템에 저장할 파일명을 만든다.(전송되는 모든 파일을 upload 폴더에 넣기 때문에 중복이 발생 할 수 있으므로 중복을 피하기 위해 원본 파일명과 저장된 파일명을 따로 관리한다)
		String saveFileName=
				System.currentTimeMillis()+orgFileName; //클라이언트에게 파일을 다운로드 시켜줄 때 필요함
		try{
			//upload 폴더에 파일을 저장한다.
