...
public class UsersDaoImpl implements UsersDao{
	...
	public boolean isExist(String inputId) {
		...
		String id=session.selectOne("users.isExist", inputId); //select 된 row 1개(selectOne)
	...
	public UsersDto getData(String id) {
		...
		return session.selectOne("users.getData", id); //변수에 담지 않고 직접 쓰는게 더 좋지만 공부하기 위해서 위해서 변수에 담아서 리턴했던 것
	}
